...
public class Random1 : MonoBehaviour
{
    ...
    void RandomRange()
    {
        ...
        //Debug.Log($"랜덤(float) = {Random.Range(0.5f, 1.5f):F2}"); //Range 안에 float를 집어넣으면 range의 범위가 float로 바뀜, :F(숫자) = float의 소숫점 (숫자)자리까지 적용한다
        ...
        // Random float 2개 출력 : circle
        // Random float 3개 출력 : sphere
    }
    ...
    void RandomRate()
    {
        ...
        while (true)
        {
            ...
            //Debug.Log(result);
            if (result != insertnum)
                Debug.Log($"랜덤값의 결과 = {result}");
            else
                Debug.Log($"{a}번째 시도 끝에, 입력한 숫자 = {result}");
